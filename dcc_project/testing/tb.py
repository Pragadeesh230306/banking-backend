...
def get_current_user():
    ...
    try:
        ...
        if len(parts) == 2:
            ...
            token = parts[0]   # raw token fallback
...
def account():
    ...
    cursor.execute("""
        SELECT account_number, account_type, account_tier, status
    ...
    cursor.execute("""
        SELECT 
            ...
            END) -
            SUM(CASE 
                WHEN from_account = ? THEN amount + charge
                ELSE 0 
            END) AS balance
        FROM transactions
...
def auto_approve_users():
    while True:
        try:
            ...
            cursor.execute("SELECT * FROM users WHERE status='INACTIVE'")
            users = cursor.fetchall()

            for u in users:
                created = datetime.datetime.fromisoformat(u["created_at"])

                if (now - created).total_seconds() >= 120:

                    acc_no = str(random.randint(1000000000, 9999999999))

                    cursor.execute("UPDATE users SET status='ACTIVE' WHERE user_id=?", (u["user_id"],))

                    cursor.execute("""
                        INSERT INTO accounts(
                            user_id, account_number, account_type,
                            account_tier, balance, status
                        )
                        VALUES (?, ?, 'SAVINGS', ?, 0, 'ACTIVE')
                    """, (u["user_id"], acc_no, u["tier"]))
...
threading.Thread(target=auto_approve_users, daemon=True).start()
# =========================================================
# LOGOUT
# =========================================================
...
# =========================================================
# ADMIN: VIEW ALL LOANS (READ ONLY)
# =========================================================
...
def admin_loans():
    ...
    if not role or role["role"].upper() != "EMPLOYEE":
